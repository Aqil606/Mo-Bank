...
const char* password = "Anonymous@123";

// Insert Firebase project API Key
#define API_KEY "AIzaSyB-zcwprPPuYw85YOB0phYmvh1tq3zmBqE"

// Insert Authorized Email and Corresponding Password
#define USER_EMAIL "testing@mobank.com"
#define USER_PASSWORD "mobank123"

// Insert Firebase storage bucket ID e.g bucket-name.appspot.com
#define STORAGE_BUCKET_ID "mobank-9efbd.appspot.com"
...
void setup() {
  ...
  if(Firebase.signUp(&configF, &auth, "", "")){
    ...
    Serial.printf("%s/n", configF.signer.signupError.message.c_str());
  }
...
void loop() {
  if (Firebase.RTDB.getBool(&fbdo, "data_mobank/IR_IN/")){
  uang_dalam = fbdo.boolData();
  }
  ...
  if(uang_dalam == true){
    ...
    Serial.print("Uploading picture... ");
      ...
      fcsUploadCallback)){
        Serial.printf("\nDownload URL: %s\n", fbdo.downloadURL().c_str());
        link_foto = fbdo.downloadURL().c_str();
        if(Firebase.ready() && signupOK && (millis() - senDataPrevMillis > 5000 || senDataPrevMillis == 0)){
          if (Firebase.RTDB.setString(&fbdo, "data_mobank/photo_link/", link_foto)){
  ...
  digitalWrite(LED, LOW);
  Serial.println("Tidak Ada Uang Didalam");
  delay(1000);
}